(() => {
  ...
  async function createProductMaster(fields) {
    ...
    const model = String(fields.model || fields.sku || "").trim();
    if (!model) throw new Error("请填写型号。");
    const category = String(fields.category || "成品").trim() || "成品";
    const unit = String(fields.unit || "件").trim() || "件";
    const default_warning_value = Math.max(0, Math.floor(safeNumber(fields.default_warning_value ?? fields.warning_level ?? 1)));
  ...
  async function createStockTransaction(payload) {
    ...
    const opLabel = String(p.operator || (await getAuthOperatorLabel())).trim();
    const src = String(p.source || "手动").trim() || "手动";
    const opType = String(p.operation_type || "入库").trim() || "入库";
    const inboundSub = String(p.inbound_subtype || p.inboundSource || "").trim();
    const row = {
  ...
  async function adjustStock(productId, type, quantity, remark, meta) {
    ...
    const opLabel = String(m.operator || (await getAuthOperatorLabel())).trim();
    ...
    const row = {
      ...
      inbound_subtype: String(m.inbound_subtype || "").trim(),
      ...
      source: src,
      operation_type: opType,
      outbound_subtype: m.outbound_subtype != null ? String(m.outbound_subtype).trim() || null : null
    };
    await insertStockLedgerRow(row);
    return { ok: true };
  }
